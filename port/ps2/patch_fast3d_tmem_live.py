...
def patch(source: str) -> str:
    ...
    source = replace_once(
        ...
        "    if ((rsp.geometry_mode & G_CULL_BOTH) != 0\n"
        "        /* Preserve Fast3D's sign-aware eye-plane culling for W < 0.\n"
        "         * It deliberately flips winding when an odd number of vertices\n"
        "         * lies behind the eye. Only defer culling for near-zero W, where\n"
        "         * the pre-divide screen-space test would be numerically unsafe;\n"
        "         * the PS2 homogeneous clipper owns that degenerate boundary. */\n"
        "        && (v1->w < -1.0e-6f || v1->w > 1.0e-6f)\n"
        "        && (v2->w < -1.0e-6f || v2->w > 1.0e-6f)\n"
        "        && (v3->w < -1.0e-6f || v3->w > 1.0e-6f)\n"

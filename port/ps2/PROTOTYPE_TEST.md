...
Pass `--file-log`, or configure a local build with
`-DPD_PS2_FILE_LOG_DEFAULT=ON`, to mirror human-readable logs to `pdps2.log`
beside the ELF. Routine CI does not publish a file-logging artifact because it
is not a valid performance or gameplay baseline.
...
frame or other hot paths. `--no-log` overrides even a locally configured
filelog diagnostic build and disables the filesystem sink.

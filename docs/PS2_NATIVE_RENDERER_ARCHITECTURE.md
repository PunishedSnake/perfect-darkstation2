...
- **REAL HARDWARE, INPUT ISOLATION:** commit `fd243162` displayed the isolated triangles and a changing heartbeat on a physical console, proving that the EE/GS frame loop remained live while `Select` produced no visible response. PAD input is therefore diagnosed independently of GS completion. The backend now reuses launcher-resident `sio2man`/`padman` modules, accepts both PS2SDK-ready states (`STABLE` and `FINDCTP1`), samples raw `Select` from both physical ports before game-action mapping, and renders a separate PAD status bar. Its progression is red=backend unavailable, orange=RPC initialised, yellow=port open, blue=transport ready, cyan=`padRead` succeeded, magenta=raw button held and white=raw `Select` held;

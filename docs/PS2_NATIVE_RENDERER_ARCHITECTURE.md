...
- **CONFIRMED:** `FRAME.FBP` addresses local memory in 8192-byte units; current gsKit likewise rounds system-buffer allocations to 8192 bytes. The project allocator now supports explicit power-of-two alignment instead of assuming every GS resource is a texture;
...
- **CONFIRMED:** after local-memory texture data changes, `TEXFLUSH` is a required texture-cache transition rather than a general-purpose GS completion fence;
...
- **CONFIRMED:** `PSMT8H` reinterprets the high byte of each 32-bit local-memory word as an indexed texel, so the alpha lane of a PSMCT32 target does not require the spatial 32-to-8-bit shuffle used for low RGB lanes;
...
- PS2 detail textures default on and retain their explicit two-tile pair. Ordinary mip LOD temporarily collapses both combiner inputs to the known-good base tile because the GS path does not yet own a complete mip chain; this preserves base-texture correctness instead of sampling absent or stale adjacent mip residency. Explicit two-level mip interpolation can return only after the full tile chain and LOD selection are hardware-validated;
...
- `PD_PS2_ALPHA_TRILERP_DIAGNOSTIC=ON` remains available as a local
  scene-selection option. The dedicated `pd-ps2-alpha-trilerp-diag.elf` was a
  historical CI artifact and is not published by routine current CI;
...
- **CURRENT IMPLEMENTATION:** the next geometry stage has a host-tested raw
  ...
  vertices and one fixed suffix slot. Unused state slots target GS NOP, keeping

...
- capture the A/B screen shown immediately by `pd-ps2-alpha-trilerp-diag.elf`: left is the live GS graph and right is the deterministic CPU reference. `Select` returns to the baseline cube. Record console model, video mode and build commit with the comparison because emulator agreement alone does not promote the gate;
